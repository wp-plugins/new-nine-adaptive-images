...
/*---------------------- We left the default values in place for reference, but we're now going -----------------------*/
/*---------------------- to load the WordPress environment and use the settings set in WordPress ----------------------*/
...
/*---Rewrite the config variables set above---*/
$wp_breakpoints = get_option( 'adaptive_images_breakpoints' );
if( $wp_breakpoints ){
    $resolutions=array();
    $breakpoints = explode( ',', $wp_breakpoints );
    foreach($breakpoints as $breakpoint){
        if( is_numeric( trim( $breakpoint ) ) ){
            $resolutions[]=trim( $breakpoint );
        }
    }
    rsort($resolutions);
}
$wp_quality = get_option( 'adaptive_images_jpg_quality' );
if( is_numeric( $wp_quality ) ){
    $jpg_quality = $wp_quality;
}
$wp_watch_cache = get_option( 'adaptive_images_watch_cache' );
if( $wp_watch_cache == 0 ){
    $watch_cache = FALSE;
}
$wp_time_cache = get_option( 'adaptive_images_time_cache' );
if( is_numeric( $wp_time_cache ) ){
    $browser_cache = $wp_time_cache;
}
$wp_sharpen = get_option( 'adaptive_images_sharpen' );
if( $wp_sharpen == 0 ){
    $sharpen = FALSE;
}
$n9m_uploads_dir_array=wp_upload_dir();
$cache_path = rtrim( ltrim( str_replace( $_SERVER['DOCUMENT_ROOT'], '', $n9m_uploads_dir_array['basedir'] ), '/' ), '/' ).'/ai-cache';

/*---------------------- We're done hijacking. The rest of this script is the original AI file. -----------------------*/
